...
describe("commitCardChecklist (ancestor feature inheritance)", () => {
  ...
  // -------------------------------------------------------------------------
  // NEO-38 — in-band TCDB applied at the setName node OVERRIDES the heuristic
  // (it runs after the seed; the heuristic value is the node's "old value",
  // which TCDB's write matches-and-overwrites down the subtree). Operator
  // overrides on a descendant survive.
  // -------------------------------------------------------------------------
  test("commit applies TCDB at setName, overriding the heuristic seed", async () => {
    const t = convexTest(schema, modules);
    const asAdmin = t.withIdentity(ADMIN_IDENTITY);
    ...
    await asAdmin.mutation(api.selectorOptions.commitCardChecklist, {
      ...
      confirmedNewPlayers: ["Player"],
      confirmedNewTeams: [],
      // TCDB says this is a reprint and gives real setMetadata. The heuristic
      // seeded isReprint="false" at setName; TCDB must override it to "true".
      tcdb: {
    ...
    // setName node: TCDB setMetadata merge-patched; isReprint overridden.
    const setNameNode = await t.run(async (ctx) => ctx.db.get(ids.setNameId));
    expect(setNameNode!.setMetadata?.tcdbSetId).toBe("27845");
    expect(setNameNode!.setMetadata?.releaseDate).toBe("2024-08-21");
    expect(setNameNode!.setMetadata?.block).toBe("Topps Chrome");
    expect(setNameNode!.features?.isReprint).toBe("true"); // TCDB > heuristic
    expect(setNameNode!.features?.manufacturer).toBe("Topps");
    expect(setNameNode!.features?.block).toBe("Topps Chrome");

    // Descendant variantType node followed the override too.
    const variantNode = await t.run(async (ctx) =>
      ctx.db.get(ids.variantTypeId),
    );
    expect(variantNode!.features?.isReprint).toBe("true");

    // The card inherited the TCDB-overridden value, not the heuristic "false".
    const cards = await t.run(async (ctx) =>
      ctx.db
        .query("cardChecklist")
        .withIndex("by_selector_option", (q) =>
    ...
    expect(cards).toHaveLength(1);
    expect(cards[0].features?.isReprint).toBe("true");
    expect(cards[0].features?.manufacturer).toBe("Topps");
  });
