...
2. **`util/util-login-to-bsc.yaml` + `util/util-login-to-sportlots.yaml` (R8 — the active flake source):** the `scrollUntilVisible` to "Test Credentials" / the SL-auth-success toast on the below-the-fold creds page **omits `centerElement: true`**. This is the exact CDP-scroll flake (`MismatchedInputException`) that failed `sets-base` **and that broke the seed (`setup.yaml`) on tonight's re-run `fdf0271`.** Fixing it at the util source fixes the warm everywhere. Also drop the 20s "Neon Binder" wait (R5) and the redundant `assertVisible` after the toast scroll (R6).
...
## ⚠️ Live signal from tonight
The `sets-base` fix (PR #52, commit `fdf0271`) is **not yet validated** — the queue re-run **failed at the seed**, not the runners: `setup.yaml` failed both attempts during its worker-0 marketplace warm (the same `util-login` center-scroll flake in #2 above). **Action:** apply the `util-login-*` `centerElement` fix (#2), then re-run the queue to validate both `setup.yaml` and `sets-base`.
...
1. **`util-login-to-bsc` + `util-login-to-sportlots` `centerElement` fix (R8).** Unblocks the seed (`setup.yaml`) + the warm everywhere. Then **re-run the queue to validate `setup.yaml` + `sets-base`.**
